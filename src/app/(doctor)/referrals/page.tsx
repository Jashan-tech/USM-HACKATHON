...
export default async function ReferralsPage({ searchParams }: PageProps) {
  ...
  if (searchParams.risk) {
    query = query.eq('risk_level', searchParams.risk);
  }
  if (searchParams.search) {
    // Note: This assumes patient name search - in a real implementation you might want to join and search on patient name
    // For now, we'll skip this filter since it requires joining with profiles table
  }
  ...
  const statusCountMap = referrals?.reduce(
    (acc, ref) => {
